...
frequencia_baixas = atipicos_baixo["TP_LOCALIZACAO_ESC"].map(tipos_escola).value_counts().sort_index()

# Plotar gráfico de barras
plt.figure(figsize=(8, 5))
frequencia_baixas.plot(kind='bar', color='cornflowerblue', edgecolor='black')
plt.title("Distribuição da localização de Escola - Notas Baixas (Outliers)")
plt.xlabel("Tipo de Escola")
plt.ylabel("Quantidade")
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig("resultados/urbanização_outliers_abaixo.png")
plt.show()

# Mapear e contar a frequência dos tipos de escola entre notas baixas
frequencia_altas = atipicos_alto["TP_LOCALIZACAO_ESC"].map(tipos_escola).value_counts().sort_index()

# Plotar gráfico de barras
plt.figure(figsize=(8, 5))
frequencia_altas.plot(kind='bar', color='cornflowerblue', edgecolor='black')
plt.title("Distribuição da loacalização de Escola - Notas Altas (Outliers)")
plt.xlabel("Tipo de Escola")
plt.ylabel("Quantidade")
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig("resultados/urbanização_outliers_acima.png")
plt.show()
